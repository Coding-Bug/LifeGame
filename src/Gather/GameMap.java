...
public class GameMap {
    ...
    private boolean[][]isChange=new boolean[Rows][Cols]; // 记录方格状态是否改变
    ...
    */
    ...
    public void reset(double lifeRatio){
        ...
        while(lifeNumber!=0){
            while(true){
                randomRow=(int)rand.nextInt(Rows-1);
                randomCol=(int)rand.nextInt(Cols-1);
    ...
    public boolean getIsChange(int row,int col){
        return isChange[row][col];
    }
    public void  setIsChange(int row,int col,boolean val){
        isChange[row][col] = val;
    }
